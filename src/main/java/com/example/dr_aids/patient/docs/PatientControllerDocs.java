...
public interface PatientControllerDocs {
    ...
    @Operation(summary = "환자 정보 저장", description = "새로운 환자 정보를 등록합니다.")
    @ApiResponse(responseCode = "200", description = "환자 정보 저장 성공",
            content = @Content(schema = @Schema(example = "{\"message\": \"환자 정보가 저장되었습니다.\"}")))
    ResponseEntity<?> savePatientInfo(@RequestBody @Parameter(description = "환자 정보 요청 DTO")
                                      @Schema(implementation = PatientInfoRequestDto.class) PatientInfoRequestDto dto);

    @Operation(summary = "환자 정보 수정", description = "환자의 기존 정보를 수정합니다.")
    @ApiResponse(responseCode = "200", description = "환자 정보 수정 성공",
            content = @Content(schema = @Schema(example = "{\"message\": \"환자 정보가 수정되었습니다.\"}")))
    ResponseEntity<?> updatePatientInfo(@PathVariable Long id,
                                        @RequestBody @Parameter(description = "수정할 환자 정보")
                                        @Schema(implementation = PatientInfoRequestDto.class) PatientInfoRequestDto dto);
    ...
    @Operation(summary = "환자 방문 여부 수정", description = "특정 환자의 당일 방문 여부를 업데이트합니다.")
    @ApiResponse(responseCode = "200", description = "환자 방문 여부 수정 성공",
            content = @Content(schema = @Schema(example = "{\"message\": \"환자 방문 여부가 수정되었습니다.\"}")))
    ResponseEntity<?> updatePatientVisitingStatus(@PathVariable Long id,
                                                  @RequestBody @Parameter(description = "방문 여부 수정 요청 DTO")
                                                  @Schema(implementation = PatientVisitindRequestDto.class)
                                                  PatientVisitindRequestDto dto);
}
